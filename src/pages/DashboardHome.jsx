...
import { frasesMotivacionais } from "../data/frases"; // <--- Importa as frases
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // --- LÓGICA DA FRASE DO DIA (Sincronizada) ---
      const hoje = new Date();
      // Criamos uma semente (seed) única para hoje: AAAAMMDD (ex: 20260218)
      const seed = hoje.getFullYear() * 10000 + (hoje.getMonth() + 1) * 100 + hoje.getDate();
      
      // Escolhemos o índice baseado na semente. 
      // O operador % garante que o número nunca ultrapassa o tamanho da lista.
  ...
  // Funções de busca (mantidas conforme o original)
  ...
  return (
    <div className="dashboard-home">
            ...
            {showMenu && (
              <div style={{ position: 'absolute', top: '110%', right: 0, background: 'white', border: '1px solid #e2e8f0', borderRadius: '10px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', minWidth: '200px', zIndex: 50, overflow: 'hidden' }}>
                <div style={{padding: '12px 15px', borderBottom: '1px solid #f1f5f9', background: '#f8fafc'}}>
                    <span style={{fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 'bold', letterSpacing: '0.5px'}}>A Minha Conta</span>
                </div>
                ...
                <Link to="/dashboard/ferias" style={{ textDecoration: 'none' }}>
                    <button className="menu-item" style={{ width: '100%', textAlign: 'left' }}>📆 Férias/Ausências</button>
                </Link>
                <div style={{borderTop: '1px solid #f1f5f9', margin: '5px 0'}}></div>
                <button className="menu-item logout" onClick={handleLogout}>🚪 Terminar Sessão</button>
              </div>
            )}
        </div>
      </div>
      ...
      {/* GRID PRINCIPAL */}
      <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', alignItems: 'start' }}>
        <div><WidgetAssiduidade /></div>
        <div><WidgetCalendar /></div>
                    ...
                    {tarefas.map((t) => (
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <span className={`badge`} style={{ fontSize: '0.65rem', padding: '2px 6px', borderRadius: '12px', background: t.prioridade === 'urgente' ? '#fee2e2' : '#e0f2fe', color: t.prioridade === 'urgente' ? '#991b1b' : '#0369a1' }}>{t.prioridade}</span>
                            </div>
