...
export default function PropostasFinanciamento({ propostaId, initialEmpresaConsultoraId = "", onEmpresaConsultoraChange }) {
  ...
  useEffect(() => {
    ...
    if (actualPropostaId && !isLoadingData) {
      void carregarPropostaExistente();
    }
  }, [actualPropostaId, isLoadingData, empresasConsultoras, clientes, tiposProj, programas]);
  ...
  const selectEmpresaConsultora = (id) => {
    ...
    if (found) {
      ...
      // Detecta tipo de proposta baseado em tem_cursos
      const tipo = found.tem_cursos ? "formacao" : "financiamento";
      setTipoProposta(tipo);

      // Callback para router saber o tipo
      if (onEmpresaConsultoraChange) {
        onEmpresaConsultoraChange(found);
      }
